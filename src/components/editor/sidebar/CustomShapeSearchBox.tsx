...
interface CustomShapeSearchBoxProps {
  searchString: string;
  onStartSearch: (kw: string) => void;
}

const CustomShapeSearchBox: FC<CustomShapeSearchBoxProps> = ({ 
  searchString, 
  onStartSearch 
}) => {
      ...
      if (keyword && keyword.trim() !== "" && config.apis) {
        try {
          const response = await axios.get(
            `${config.apis.url}${
              config.apis.shapeKeywordSuggestion
          ...
          console.error("Error fetching shape suggestions:", error);
          setSuggestItems([]);
        }
      } else {
        setSuggestItems([]);
...
export default CustomShapeSearchBox;

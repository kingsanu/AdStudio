...
if [ "$(id -u)" -ne 0 ]; then
  error "This script must be run as root. Try 'sudo ./deploy-vps.sh'"
fi
...
# Get MongoDB URI
read -p "Enter MongoDB URI (default: mongodb://localhost:27017/canva-editor): " MONGODB_URI
MONGODB_URI=${MONGODB_URI:-mongodb://localhost:27017/canva-editor}
...
if ! command -v mongod &> /dev/null; then
  ...
  apt install -y mongodb || error "Failed to install MongoDB"
  systemctl start mongodb
  systemctl enable mongodb
else
  info "MongoDB is already installed: $(mongod --version | head -n 1)"
fi
...
cat > $APP_DIR/api/.env << EOL
PORT=4000
MONGODB_URI=$MONGODB_URI
EOL

info "Created .env file for API"
...
server {
    ...
    location /api {
        proxy_pass https://adstudioserver.foodyqueen.com;
        proxy_http_version 1.1;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host \$host;
